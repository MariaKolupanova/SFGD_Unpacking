...
int main ()
{
    ...
    while (temp<NumberOfFEB && !finDatLG.eof()) {
        ...
        //cout << FEBcalib[temp][0] << " " << FEBcalib[temp][1] << " " << FEBcalib[temp][2] << " " << FEBcalib[temp][3] << " " << FEBcalib[temp][4] << " " << FEBcalib[temp][5] << endl;
        temp++;
    }   
    finDatLG.close();
  ...
  for (int ih = 0; ih<FEBnumbers.size();ih++) {
        ...
        for (Int_t ich=0; ich < 96;ich++){
            if (LG_vs_ToT[FEBnumbers.at(ih)][ich]->GetEntries()>1) {
                ...
                Double_t fitPar[6];
                TF1 *f = new TF1("f",bestfit,FEBcalib[FEBnumbers.at(ih)][2],FEBcalib[FEBnumbers.at(ih)][3],5);

                Double_t fitLin1[2];
                TF1 *fline1 = new TF1("fline1",lineFit,FEBcalib[FEBnumbers.at(ih)][0],FEBcalib[FEBnumbers.at(ih)][1],2);
                fline1->SetParLimits(1,0.,20);

                Double_t fitLin2[2];
                TF1 *fline2 = new TF1("fline2",lineFit,FEBcalib[FEBnumbers.at(ih)][4],FEBcalib[FEBnumbers.at(ih)][5],2);
                fline2->SetParLimits(1,130,700);
