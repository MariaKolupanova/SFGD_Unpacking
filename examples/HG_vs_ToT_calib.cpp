...
int main ()
{
  ...
  Double_t fitPar[6];
  TF1 *f = new TF1("f",bestfit,7,22,5);

  Double_t fitLin1[2];
  TF1 *fline1 = new TF1("fline1",lineFit,4,8,2);
  fline1->SetParLimits(1,0.,200);

  Double_t fitLin2[2];
  TF1 *fline2 = new TF1("fline2",lineFit,20,23,2);
        ...
        for (Int_t enu = 0; enu<nentries[FEBnumbers.at(ih)]; enu++){
            FEBtree[FEBnumbers.at(ih)]->GetEntry(enu);
                for (Int_t i = 0; i<FEB[FEBnumbers.at(ih)].hitTimeDif->size(); i++){
                    //if (FEB[FEBnumbers.at(ih)].hitAmplMatched->at(i)>0 && FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>0 && FEB[FEBnumbers.at(ih)].hitAmplMatched->at(i) < 2500) {
                    if (FEB[FEBnumbers.at(ih)].hitAmpl->at(i)>0 && FEB[FEBnumbers.at(ih)].hitTimeDif->at(i)>0 && FEB[FEBnumbers.at(ih)].hitAmpl->at(i) < 4000) {
